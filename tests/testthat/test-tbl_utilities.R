...
test_that(".abbreviate_names works", {
    x <- setNames(nm = character())
    expect_equal(.abbreviate_names(x), character(0))

    x <- setNames(nm = NA_character_)
    expect_equal(.abbreviate_names(x), NA_character_)

    x <- setNames(nm = c("a", NA_character_))
    expect_equal(.abbreviate_names(x), c("a", NA_character_))

    x <- "a"
    expect_error(.abbreviate_names(x), "!is.null\\(names\\(x\\)\\) is not TRUE")

    x <- setNames(nm = c("b_a", "c_a"))
    expect_equal(.abbreviate_names(x), c("b_a", "c_a"))

    x <- setNames(nm = c("a_b", "a_c"))
    expect_equal(.abbreviate_names(x), c("b", "c"))

    x <- setNames(nm = c("a_b", "a.c"))
    expect_equal(.abbreviate_names(x), c("b", "c"))

    x <- setNames(nm = c("a_b_c", "d_b_c"))
    expect_equal(.abbreviate_names(x), c("a_b_c", "d_b_c"))

    ## '_' and '.' are word separators (from '[:punct:]')
    x <- setNames(nm = c("a_b_c", "a_d.c"))
    expect_equal(.abbreviate_names(x), c("b_c", "d.c"))

    ## '_' and '.' are treated differently in terms of matches
    x <- setNames(nm = c("a_b_c", "a_b.c", "a_b__c"))
    expect_equal(.abbreviate_names(x), c("b_c", "b.c", "b__c"))

    ## trailing punctuation
    x <- setNames(nm = c("b_c", "b_c_"))
    expect_equal(.abbreviate_names(x), c("c", "c_"))

    ## suffix of one matches all of another
    x <- setNames(nm = c("a.b.c", "b.c"))
    expect_equal(.abbreviate_names(x), unname(x))
})
